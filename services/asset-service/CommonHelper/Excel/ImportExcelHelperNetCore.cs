    ...
    public class ImportExcelHelperNetCore<T> where T : class
    {
        ...
        public ReponseImport<T> Import()
        {
            ...
            for (int i = StartRow; i <= rowCount; i++)
            {
                ...
                int checkEndFile = 0;
                var MessErr = "";
                for (int c = 1; c < ConfigColumn.Count + 1; c++)
                {
                    //ĐỐi tượng config cho cột
                    var objConfig = ConfigColumn[c - 1];
        ...
        public ReponseImport<T> Import(ExcelPackage package, string sheetName = null)
        {
            string ListExtensionAllow = ".xls,.xlsx";
            var result = new ReponseImport<T>();
            result.Status = true;
            ...
            if (!string.IsNullOrWhiteSpace(sheetName))
            {
                ...
                {
                    result.Status = false;
                    result.Message = $"Không tìm thấy sheet: {sheetName}";
            ...
            else
            {
                ...
                {
                    result.Status = false;
                    result.Message = "File Excel không có sheet nào";
                    return result;
                }
            }
            ...
            for (int i = StartRow; i <= rowCount; i++)
            {
                ...
                for (int c = 2; c < ConfigColumn.Count + 2; c++)
                {
                    ...
                    //Khởi tạo đối tượng đúng
                    var excelComlumnData = workSheet.Cells[i, c - 1].Value;
                    string vl = excelComlumnData != null ? excelComlumnData.ToString() : "";
                    if (vl.ToUpper().Equals("NULL"))
                    {
                        vl = "";
                    ...
                    if (!string.IsNullOrEmpty(vl))
                    {
                        switch (objConfig.TypeValue)
                        {
                            case ConstantData.TYPE_STRING:
                                ...
                                break;
                            case ConstantData.TYPE_INT:
                                if (vl.Equals("Nam", StringComparison.OrdinalIgnoreCase))
                                {
                                    vl = "0";
                                }
                                if (vl.Equals("Nữ", StringComparison.OrdinalIgnoreCase))
                                {
                                    vl = "1";
                                }
                                var rsCheckValue = vl.CheckValueInt();
