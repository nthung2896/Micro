...
namespace Hinet.Service.RabbitMQ
{
    public class UserSyncConsumerService : BackgroundService
    {
        ...
        private const string AssetQueue = "asset.service.user.sync";
        ...
        {
            _logger.LogInformation("🚀 [AssetService - RabbitMQ Consumer] Starting UserSyncConsumer background service...");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        // Giữ luồng background chạy cho đến khi token bị cancel
                ...
                {
                    _logger.LogWarning("⚠️ [AssetService - RabbitMQ Consumer] Connection lost or not available: {Msg}. Retrying in 10s...", ex.Message);
                    Cleanup();
                    try
                    {
            ...
            Cleanup();
            _logger.LogInformation("🛑 [AssetService - RabbitMQ Consumer] Background service stopped.");
        }
        ...
        private bool TryConnectAndConsume(CancellationToken stoppingToken)
        {
            ...
            var factory = new ConnectionFactory
            {
                ...
                ClientProvidedName = $"AssetService-UserConsumer-{Environment.MachineName}"
            };
            ...
            consumer.Received += (sender, ea) =>
            {
                try
                {
                    ...
                    _logger.LogInformation("📥 [AssetService - RabbitMQ] Received event '{RoutingKey}': {Body}", routingKey, message);
                ...
                {
                    _logger.LogError(ex, "❌ [AssetService - RabbitMQ] Error processing event: {Msg}", ex.Message);
                    try
                    {
            ...
            _logger.LogInformation("✅ [AssetService - RabbitMQ Consumer] Listening on queue '{Queue}' bound to '{Exchange}'", AssetQueue, UserExchange);
            return true;
        }
        ...
        private void ProcessEvent(string routingKey, string jsonMessage)
        {
            ...
            if (userRepo == null)
            {
                _logger.LogWarning("⚠️ [AssetService - RabbitMQ] IRepository<AppUser> not found in scope");
                return;
            }
            ...
            if (existingUser == null)
            {
                ...
                _logger.LogInformation("🎉 [AssetService - RabbitMQ Sync] ✅ Synced NEW User @{UserName} (Id: {Id}) into Base_TaiSan database!", userName, userId);
            }
            else
            {
                ...
                userRepo.SaveAsync().GetAwaiter().GetResult();
                _logger.LogInformation("🔄 [AssetService - RabbitMQ Sync] 🔄 Updated User @{UserName} in Base_TaiSan database.", userName);
            }
                ...
                if (roleCodes.Count > 0)
                {
                    foreach (var code in roleCodes)
                    {
                        ...
                        if (!hasUserRole)
                        {
                            ...
                            _logger.LogInformation("🔑 [AssetService - RabbitMQ Sync] Mapped User @{UserName} to Role {RoleCode} in Base_TaiSan", userName, code);
                        }
                    }
                }
            }
        ...
        private void ProcessDepartmentEvent(string routingKey, JsonElement root, IServiceScope scope)
        {
            ...
            if (deptRepo == null)
            {
                _logger.LogWarning("⚠️ [AssetService - RabbitMQ] IRepository<Department> not found in scope");
                return;
            }
            ...
            if (routingKey.EndsWith("deleted"))
            {
                ...
                if (existing != null)
                {
                    ...
                    _logger.LogInformation("🗑️ [AssetService - RabbitMQ Sync] Soft-deleted Department '{Code}' in Base_TaiSan", code);
                }
                return;
            }
            ...
            if (dept == null)
            {
                ...
                deptRepo.Add(dept);
                _logger.LogInformation("🏢 [AssetService - RabbitMQ Sync] ✅ Synced NEW Department '{Name}' ({Code}) into Base_TaiSan!", name, code);
            }
            else
            {
                ...
                _logger.LogInformation("🏢 [AssetService - RabbitMQ Sync] 🔄 Updated Department '{Name}' ({Code}) in Base_TaiSan", name, code);
            }
            deptRepo.SaveAsync().GetAwaiter().GetResult();
        }
