  ...
  const handleDeleteMenu = async (id: string) => {
    try {
      ...
      if (response.status) {
        ...
        message.error(response.message || "Xóa menu thất bại");
      }
    } catch (error: any) {
        ...
        message.success(
          `${
            response.data?.isActive
              ? "Mở khóa menu thành công"
              : "Khóa menu thành công"
          } `
        );
        handleGetListMenu();
      } else {
...
export default withAuthorization(QLMenuNav, "");
