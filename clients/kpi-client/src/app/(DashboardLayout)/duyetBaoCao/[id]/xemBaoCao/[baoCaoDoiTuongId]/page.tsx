...
const ManagerViewReportDetailPage: React.FC<PageProps> = ({ params }) => {
  ...
  const fetchSubmissionDetail = useCallback(async () => {
    if (!baoCaoDoiTuongId) return;
    try {
      const response = await bcSubmissionService.getDetail(baoCaoDoiTuongId);
      if (response?.data) {
        setSubmissionDetail(response.data);
      }
    } catch (error) {
      console.error("Lỗi khi tải chi tiết nộp báo cáo:", error);
    }
  }, [baoCaoDoiTuongId]);

  const handleApprove = async () => {
    if (submissionDetail?.status !== "SUBMITTED") {
      message.warning("Chỉ được duyệt báo cáo ở trạng thái Đã nộp");
      return;
    }
    setApproving(true);
    try {
      const response = await apiService.put(
        `/BCBaoCaoDoiTuong/UpdateStatus/${baoCaoDoiTuongId}?status=APPROVED`,
      );
      if (response.status) {
  ...
  useEffect(() => {
    fetchDetail();
    fetchSubmissionDetail();
  }, [fetchDetail, fetchSubmissionDetail]);
      ...
      >
        <div style={{ display: "flex", gap: 8 }}>
          <Button
            ...
            onClick={() => router.push(`/duyetBaoCao/${id}`)}
      ...
      >
        <Descriptions column={{ xs: 1, sm: 2, md: 3 }}>
          ...
          <Descriptions.Item label="Gửi Email">
            {detailData?.isGuiMail
              ? "Gửi mail nhắc nhở"
              : "Không gửi mail nhắc nhở"}
          </Descriptions.Item>
