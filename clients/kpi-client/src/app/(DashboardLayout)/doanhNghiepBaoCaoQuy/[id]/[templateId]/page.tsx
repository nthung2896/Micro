...
const ReportSubmissionPage: React.FC<PageProps> = ({
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      router.push(`/doanhNghiepBaoCaoQuy`);
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Xác định trạng thái dựa trên quý của báo cáo đang nộp
      // Deadline: 15 ngày đầu tháng 1 của quý tiếp theo
      // Q1 -> deadline 15/4, Q2 -> 15/7, Q3 -> 15/10, Q4 -> 15/1 năm sau
      const reportedQuarter = month ? parseInt(month) : null;
      const reportedYear = year ? parseInt(year) : today.getFullYear();

      let isLate = false;
      if (reportedQuarter) {
        // Tháng đầu tiên của quý tiếp theo (0-indexed)
        const deadlineMonth = reportedQuarter === 4 ? 0 : reportedQuarter * 3; // Q1->3(Apr), Q2->6(Jul), Q3->9(Oct), Q4->0(Jan)
        const deadlineYear = reportedQuarter === 4 ? reportedYear + 1 : reportedYear;
  ...
  return (
    <>
      ...
      <Card
        title={`Làm báo cáo: ${templateData?.name || ""} ${month ? `(Quý ${month}/${year})` : ""}`}
        bordered={false}
