...
interface ModalNhapChiTietSanPhamProps {
  visible: boolean;
  onCancel: () => void;
...
const ModalNhapChiTietSanPham: React.FC<ModalNhapChiTietSanPhamProps> = ({
  visible,
  onCancel,
  onSave,
  initialData,
  ...
  useEffect(() => {
    if (visible) {
      const initValues = {
        ...
        chamDiemSoLuong_HoanThanh: round2(initialData?.chamDiemSoLuong_HoanThanh),
  ...
  const calculateScores = (changedValues: any, allValues: any) => {
    ...
    if ("chamDiemSoLuong_HoanThanh" in changedValues || "diemTheoBoTieuChi" in changedValues) {
      ...
      const diemPct = maxScore > 0 ? (ht / maxScore) * 100 : 0;
      updates.chamDiemSoLuong_HoanThanh = Number(ht.toFixed(2));
      updates.chamDiemSoLuong_Diem = Number(Math.max(0, Math.min(100, diemPct)).toFixed(2));
    }
  ...
  const handleFinish = (values: any) => {
    ...
    const allValues = {
      ...
      chamDiemSoLuong_HoanThanh: round2(rawValues.chamDiemSoLuong_HoanThanh),
  ...
  const handleSelectTieuChi = (records: any[]) => {
    if (records && records.length > 0) {
      ...
      const updates: any = {
        ...
        chamDiemSoLuong_HoanThanh: Number(diemCoSo.toFixed(2)),
        chamDiemSoLuong_KhongHoanThanh: 0,
          ...
          <Row gutter={8}>
            ...
            <Col span={10}>
              <Form.Item
                ...
                label={<span style={{ fontWeight: 600 }}>Điểm theo Bộ tiêu chí</span>}
                style={{ marginBottom: "10px" }}
              >
                <InputNumber
                  style={{ width: "100%" }}
                ...
                {selectedTieuChiName ? (
                  <>
                    <Tag
                      ...
                      style={{ whiteSpace: 'normal', height: 'auto', padding: '2px 6px', fontSize: '13px' }}
                    >
                      {selectedTieuChiName}
                    </Tag>
                    <Button
            ...
            <Row gutter={8}>
              <Col span={8}>
                ...
                <Form.Item name="chamDiemSoLuong_HoanThanh" label={<span style={{ fontWeight: 600 }}>SL Hoàn thành</span>} style={{ marginBottom: "8px" }}>
                  <InputNumber precision={2} style={{ width: "100%" }} min={0} step={0.01} placeholder="HT" />
                </Form.Item>
                <Form.Item name="chamDiemSoLuong_KhongHoanThanh" label={<span style={{ fontWeight: 600 }}>Không hoàn thành</span>} style={{ marginBottom: "8px" }}>
                  <InputNumber precision={2} style={{ width: "100%" }} min={0} step={0.01} formatter={formatScore} placeholder="0" />
                </Form.Item>
                <Form.Item name="chamDiemSoLuong_Diem" label={<span style={{ fontWeight: 600 }}>Điểm (%) Số lượng</span>} style={{ marginBottom: "4px" }}>
                  <InputNumber precision={2} style={{ width: "100%" }} disabled placeholder="0%" formatter={formatPercent} />
                </Form.Item>
              </Col>
              ...
              <Col span={8}>
                ...
                <Form.Item name="chamDiemChatLuong_KhongDat" label={<span style={{ fontWeight: 600 }}>Số lần không đạt</span>} style={{ marginBottom: "8px" }}>
                  <InputNumber precision={2} style={{ width: "100%" }} min={0} step={1} placeholder="Số lần" />
                </Form.Item>
                <Form.Item name="chamDiemChatLuong_SoDiemConLai" label={<span style={{ fontWeight: 600 }}>Số điểm còn lại</span>} style={{ marginBottom: "8px" }}>
                  <InputNumber precision={2} style={{ width: "100%" }} min={0} placeholder="0" formatter={formatScore} disabled />
                </Form.Item>
...
export default ModalNhapChiTietSanPham;
