"use client";

import React, { useCallback, useEffect, useState } from "react";
import { Button, Card, Col, Form, Input, Row, Select, DatePicker } from "antd";
import {
  DownOutlined,
  FileExcelOutlined,
  ReloadOutlined,
  SearchOutlined,
  UpOutlined,
} from "@ant-design/icons";
import Flex from "@/components/shared-components/Flex";
import { useForm } from "antd/es/form/Form";
import userService from "@/services/user/user.service";
import { useSelector } from "@/store/hooks";
import ConstractStatusConstant from "@/constants/ConstractStatusConstant";
import { AuthenticationContractSearchType } from "@/types/authenticationContract/request";

export interface ContractSearchValues extends AuthenticationContractSearchType {
  createdDateRange?: any;
}

interface ContractSearchProps {
  onSearch: (values: ContractSearchValues) => void;
  onReset: () => void;
  onExport?: () => void;
  isDoanhNghiep?: boolean;
}

const ContractSearch: React.FC<ContractSearchProps> = ({
  onSearch,
  onReset,
  onExport,
  isDoanhNghiep,
}) => {
  const [form] = useForm<ContractSearchValues>();
  const [isExpanded, setIsExpanded] = useState(false);

  const [specialistOptions, setSpecialistOptions] = useState<{ value: string; label: string }[]>([]);
  const [loadingSpecialists, setLoadingSpecialists] = useState(false);

  // Lấy vai trò hiện tại
  const authState = useSelector((state: any) => state.auth);
  const currentUser = authState?.User;
  const userRoles: string[] = currentUser?.listRole || authState?.ListRole || [];
  const isDN = userRoles.includes("DoanhNghiep") || isDoanhNghiep === true;

  // Fetch chuyên viên
  const fetchSpecialists = useCallback(async () => {
    if (isDN) return;
    setLoadingSpecialists(true);
    try {
      const hasCucRole = userRoles.some((r) => r.includes("Cuc"));
      const vaiTro = hasCucRole
        ? ["ChuyenVienCuc"]
        : ["ChuyenVienSo", "ChuyenVienCuc"];

      const response = await userService.getUserByRole({
        pageIndex: 1,
        pageSize: 1000,
        vaiTro,
      });
      const itemsList = response?.data?.items || [];
      const opts = itemsList
        .filter((u: any) => u?.id)
        .map((u: any) => ({
          value: u.id,
          label:
            u.name && u.userName
              ? `${u.name} (${u.userName})`
              : u.name || u.userName || u.id,
        }));

      setSpecialistOptions([
        { value: "00000000-0000-0000-0000-000000000000", label: "⚠️ Chưa phân công" },
        ...opts,
      ]);
    } catch (err) {
      console.error("Không tải được danh sách chuyên viên xử lý:", err);
    } finally {
      setLoadingSpecialists(false);
    }
  }, [isDN, userRoles]);

  useEffect(() => {
    fetchSpecialists();
  }, [fetchSpecialists]);

  const handleReset = () => {
    form.resetFields();
    onReset();
  };

  const handleFormFinish = (values: any) => {
    const searchParams: ContractSearchValues = { ...values };

    if (values.createdDateRange && values.createdDateRange.length === 2) {
      searchParams.createdDateFrom = values.createdDateRange[0].format("YYYY-MM-DD");
      searchParams.createdDateTo = values.createdDateRange[1].format("YYYY-MM-DD");
    }

    onSearch(searchParams);
  };

  return (
    <Card className="customCardShadow mb-3" style={{ borderRadius: 12, border: "1px solid #e2e8f0" }}>
      <Form
        form={form}
        layout="vertical"
        name="contract-search-shared"
        onFinish={handleFormFinish}
        autoComplete="off"
      >
        <Row gutter={[24, 0]}>
          {/* Tên nền tảng */}
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item label="Tên nền tảng" name="name">
              <Input
                placeholder="Nhập tên nền tảng..."
                allowClear
                style={{ borderRadius: 6 }}
              />
            </Form.Item>
          </Col>

          {/* Tên miền */}
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item label="Tên miền" name="domain">
              <Input
                placeholder="Nhập tên miền..."
                allowClear
                style={{ borderRadius: 6 }}
              />
            </Form.Item>
          </Col>

          {/* Chủ sở hữu */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item label="Chủ sở hữu" name="chuSoHuu">
                <Input
                  placeholder="Nhập chủ sở hữu..."
                  allowClear
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}

          {/* Người đại diện */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item label="Người đại diện" name="representerNameOnline">
                <Input
                  placeholder="Nhập tên người đại diện..."
                  allowClear
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}

          {/* Mã số thuế (Chỉ hiển thị cho cán bộ) */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item label="Mã số thuế doanh nghiệp" name="companyTaxCode">
                <Input
                  placeholder="Nhập mã số thuế..."
                  allowClear
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}

          {/* Khoảng ngày tạo hồ sơ */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item label="Khoảng ngày tạo hồ sơ" name="createdDateRange">
                <DatePicker.RangePicker
                  format="DD/MM/YYYY"
                  placeholder={["Từ ngày", "Đến ngày"]}
                  style={{ width: "100%", borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}

          {/* Trạng thái hồ sơ */}
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item label="Trạng thái hồ sơ" name="status">
              <Select
                allowClear
                placeholder="-- Chọn trạng thái --"
                options={ConstractStatusConstant.getDropdownListKey()}
                style={{ borderRadius: 6 }}
              />
            </Form.Item>
          </Col>

          {/* ── BỘ LỌC NÂNG CAO (Ẩn/Hiện) ───────────────────────── */}

          {/* Số điện thoại đại diện (nâng cao) */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24} style={{ display: isExpanded ? "block" : "none" }}>
              <Form.Item label="Số điện thoại liên hệ đại diện" name="representerMobileOnline">
                <Input
                  placeholder="Nhập số điện thoại..."
                  allowClear
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}

          {/* Email đại diện (nâng cao) */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24} style={{ display: isExpanded ? "block" : "none" }}>
              <Form.Item label="Email người đại diện" name="representerEmailOnline">
                <Input
                  placeholder="Nhập email đại diện..."
                  allowClear
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}



          {/* Chuyên viên xử lý (Chỉ hiển thị cho cán bộ, nâng cao) */}
          {!isDN && (
            <Col xl={8} lg={8} md={12} xs={24} style={{ display: isExpanded ? "block" : "none" }}>
              <Form.Item label="Cán bộ xử lý" name="chuyenVienXuLyId">
                <Select
                  allowClear
                  placeholder="Chọn cán bộ xử lý"
                  loading={loadingSpecialists}
                  showSearch
                  optionFilterProp="label"
                  options={specialistOptions}
                  filterOption={(input, option) =>
                    ((option?.label as string) ?? "")
                      .toLowerCase()
                      .includes(input.toLowerCase())
                  }
                  style={{ borderRadius: 6 }}
                />
              </Form.Item>
            </Col>
          )}
        </Row>

        {/* ── FOOTER BUTTONS ────────────────────────────────────────────── */}
        <Flex
          alignItems="center"
          justifyContent="center"
          style={{ gap: 12, marginTop: 12 }}
        >
          <Button
            color="cyan" variant="solid"
            htmlType="submit"
            icon={<SearchOutlined />}
            size="middle"
            style={{
              borderRadius: 8,
              fontWeight: 600,
              padding: "0 24px",
              height: 38,
              border: "none",
              boxShadow: "0 4px 12px rgba(24, 144, 255, 0.2)",
            }}
          >
            Tìm kiếm
          </Button>

          <Button
            icon={<ReloadOutlined />}
            size="middle"
            onClick={handleReset}
            style={{ borderRadius: 8, fontWeight: 600, padding: "0 24px", height: 38 }}
          >
            Làm mới
          </Button>

          {!isDN && (
            <Button
              type="default"
              onClick={() => setIsExpanded(!isExpanded)}
              icon={isExpanded ? <UpOutlined /> : <DownOutlined />}
              size="middle"
              style={{ borderRadius: 8, height: 38 }}
            >
              {isExpanded ? "Thu gọn bộ lọc" : "Mở rộng bộ lọc"}
            </Button>
          )}

          {onExport && (
            <Button
              type="default"
              onClick={onExport}
              icon={<FileExcelOutlined />}
              size="middle"
              style={{ borderRadius: 8, height: 38, color: "#16a34a", borderColor: "#16a34a" }}
            >
              Xuất Excel
            </Button>
          )}
        </Flex>
      </Form>
    </Card>
  );
};

export default ContractSearch;
