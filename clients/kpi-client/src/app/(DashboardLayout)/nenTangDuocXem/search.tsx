...
import { DangKyXemNenTangSearchType } from "@/types/dang-ky-xem/request";
import { useSelector } from "@/store/hooks";
import userService from "@/services/user/user.service";
import PlatformStatusConstant from "@/constants/PlatformStatusConstant";

export interface NenTangDuocXemSearchProps {
  activeTab?: string;
  activeTabKey?: string;
  isStaff?: boolean;
  isDoanhNghiep?: boolean;
  onFinish: (values: DangKyXemNenTangSearchType) => void;
  onExport?: () => void;
}

const NenTangDuocXemSearch: React.FC<NenTangDuocXemSearchProps> = ({
  activeTab,
  activeTabKey,
  isStaff,
  isDoanhNghiep,
  ...
  const [form] = useForm<DangKyXemNenTangSearchType>();
  ...
  const handleFormFinish = (values: any) => {
    const searchParams: DangKyXemNenTangSearchType = { ...values };

    if (values.displayDateFrom) {
      searchParams.displayDateFrom = values.displayDateFrom.format("YYYY-MM-DD");
    }
    if (values.displayDateTo) {
      searchParams.displayDateTo = values.displayDateTo.format("YYYY-MM-DD");
    }
      ...
      >
        <Row gutter={[24, 0]}>
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item<DangKyXemNenTangSearchType> label="Tên nền tảng" name="name">
              <Input
          ...
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item<DangKyXemNenTangSearchType> label="Địa chỉ tên miền / Website" name="domain">
              <Input
          ...
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item<DangKyXemNenTangSearchType> label="Trạng thái hồ sơ" name="status">
              <Select
          ...
          {isStaffMode && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item<DangKyXemNenTangSearchType> label="Doanh nghiệp chủ quản" name="companyName">
                <Input
          ...
          {isStaffMode && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item<DangKyXemNenTangSearchType> label="Mã số thuế" name="companyTaxCode">
                <Input
          ...
          {isStaffMode && (
            <Col xl={8} lg={8} md={12} xs={24}>
              <Form.Item<DangKyXemNenTangSearchType> label="Người nhận xử lý" name="reviewId">
                <Select
                  allowClear
          ...
          <Col xl={8} lg={8} md={12} xs={24}>
            <Form.Item label="Thời gian hiển thị" style={{ marginBottom: 0 }}>
              <Form.Item<DangKyXemNenTangSearchType>
                name="displayDateFrom"
                style={{ display: 'inline-block', width: 'calc(50% - 12px)', marginBottom: 24 }}
              >
                <DatePicker
                  format="DD/MM/YYYY"
                  placeholder="Từ ngày"
...
export default NenTangDuocXemSearch;
