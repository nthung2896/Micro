...
const getExcelTrangThaiPhieu = (trangThai?: string) => {
  const labels: Record<string, string> = {
    KhoiTao: "Khởi tạo",
    GuiCap1: "Chờ cấp trên trực tiếp duyệt",
    GuiCap2: "Chờ lãnh đạo đơn vị duyệt",
...
const KPI_PhieuDanhGiaCaNhan2Page: React.FC = () => {
  ...
  const handleThuHoiSubmit = async () => {
    ...
    try {
      ...
      const response = await kPI_PhieuDanhGiaService.thuHoiPhieu({
        idPhieuDanhGia: thuHoiModalState.idPhieuDanhGia,
        idNguoiThuHoi: user.id,
        ghiChu: thuHoiModalState.ghiChu || "Cá nhân thu hồi phiếu đánh giá",
      });
      if (response && response.status !== false) {
        ...
        toast.error(response?.message || "Thu hồi phiếu đánh giá thất bại!");
      }
    } catch (error: any) {
      console.error("Lỗi khi thu hồi phiếu:", error);
  ...
  const renderTrangThaiPhieu = (trangThai?: string) => {
    ...
    switch (trangThai) {
      ...
      case "GuiCap1":
        return <Tag color="warning" style={tagStyle}>Chờ cấp trên trực tiếp duyệt</Tag>;
      case "GuiCap2":
        return <Tag color="purple" style={tagStyle}>Chờ lãnh đạo đơn vị duyệt</Tag>;
  ...
  const tableColumns: TableProps<any>["columns"] = [
      ...
      render: (_: any, record: any) => (
        <Space direction="vertical" size={10} style={{ width: "100%", padding: "4px 0" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
            <span style={{ fontWeight: 600, color: "#475569", fontSize: "13px", width: "70px", flexShrink: 0, paddingTop: "6px" }}>Chung:</span>
            ...
            ) : <span style={{ color: "#9ca3af", paddingTop: "6px", fontSize: "13px" }}>-</span>}
          </div>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
            <span style={{ fontWeight: 600, color: "#475569", fontSize: "13px", width: "70px", flexShrink: 0, paddingTop: "6px" }}>Nhiệm vụ:</span>
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space direction="vertical" size={0}>
          <span style={{ fontSize: "12px", color: "#64748b" }}>Từ: {extensions.formatDate(record.thoiGianBatDau)}</span>
          <span style={{ fontSize: "12px", color: "#64748b" }}>Đến: {extensions.formatDate(record.thoiGianKetThuc)}</span>
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        const canEditCaNhan = activeTab === "danhGiaTheoDot" && isKhoiTaoOrTraVe;
        const canEvaluateChoXuLy = activeTab === "choXuLy";

        if (canEditCaNhan || canEvaluateChoXuLy) {
          // 👉 Nút chỉnh sửa / đánh giá mới dẫn tới trang DanhGiaMultiCap (hoặc DanhGiaTCCB)
          items.push({
            label: <span style={{ color: "#0284c7", fontWeight: "600" }}>{canEvaluateChoXuLy ? "Đánh giá & Chấm điểm" : "Đánh giá theo nhiệm vụ kê khai"}</span>,
            ...
            onClick: async () => {
              ...
              try {
                dispatch(setIsLoading(true));
                const res = await kPI_PhieuDanhGiaService.checkIsVuToChucCanBo();
                if (res?.data) {
                  router.push(
                    `/kPI_PhieuDanhGia/DanhGiaMultiCap/${targetId}?idDotDanhGia=${record.idDotDanhGia}&idPhieu=${record.idPhieuDanhGia || ""}&idLyLich=${record.idLyLich || ""}`
                  );
                } else {
                  router.push(
                    `/kPI_PhieuDanhGia/DanhGiaMultiCap/${targetId}?idDotDanhGia=${record.idDotDanhGia}&idPhieu=${record.idPhieuDanhGia || ""}&idLyLich=${record.idLyLich || ""}`
                ...
                console.error("Lỗi kiểm tra quyền TCCB:", e);
                router.push(
                  `/kPI_PhieuDanhGia/DanhGiaMultiCap/${targetId}?idDotDanhGia=${record.idDotDanhGia}&idPhieu=${record.idPhieuDanhGia || ""}&idLyLich=${record.idLyLich || ""}`
                );
              } finally {
          ...
          items.push({
            label: <span style={{ color: "#16a34a", fontWeight: "600" }}>Đánh giá bộ tiêu chí chung</span>,
            key: "action_danhgia_bo_tieu_chi",
            icon: <EditOutlined style={{ color: "#16a34a" }} />,
            onClick: () => {
              const targetId = record.idPhieuDanhGia || record.idDotDanhGia;
              router.push(
                `/kPI_PhieuDanhGia/DanhGiaTCCB/${targetId}?idDotDanhGia=${record.idDotDanhGia}&idPhieu=${record.idPhieuDanhGia || ""}&idLyLich=${record.idLyLich || ""}`
              );
            },
          });
        }
        ...
        const canThuHoiCaNhan =
          (activeTab === "danhGiaTheoDot" || activeTab === "daXuLy") &&
          record.idPhieuDanhGia &&
          record.trangThai &&
          !isKhoiTaoOrTraVe &&
          record.trangThai !== "DaDuyet";
        if (canThuHoiCaNhan) {
          items.push({
  ...
  const currentTabFilter = useMemo(() => {
    const lyLich = user?.idLyLich ? { idLyLich: user.idLyLich } : {};
    if (activeTab === "danhGiaTheoDot") {
      return { ...lyLich };
    }
    if (activeTab === "choXuLy") {
      return { ...lyLich, idNguoiXuLy: user?.id, isXuLy: false, isKhacHoanThanh: true };
    }
    if (activeTab === "daXuLy") {
      return { ...lyLich, idNguoiXuLy: user?.id, isXuLy: true, isKhacHoanThanh: true };
    }
    if (activeTab === "daDuyet") {
      return { ...lyLich, idNguoiXuLy: user?.id, isXuLy: true, isKhacHoanThanh: false };
    }
    return { ...lyLich };
        ...
        items={[
          {
            key: "danhGiaTheoDot",
            label: (
              <Tooltip placement="top" title="Danh sách các đợt đánh giá áp dụng cho cá nhân để thực hiện lập phiếu và tự chấm điểm theo quy trình mới.">
                <Space>
                  <span style={{ fontWeight: 600 }}>Đánh giá theo đợt (Mới)</span>
...
export default withAuthorization(KPI_PhieuDanhGiaCaNhan2Page, "");
