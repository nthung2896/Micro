...
interface SearchProps {
  onFinish: ((values: VuViecPhanAnhSearchType) => void) | undefined;
  pageIndex: number;
  pageSize: number;
}

const Search: React.FC<SearchProps> = ({ onFinish, pageIndex, pageSize }) => {
  const [form] = useForm<VuViecPhanAnhSearchType>();
      ...
      >
        <Row gutter={24}>
          <Col xl={6} lg={8} md={12} xs={24}>
            <Form.Item<VuViecPhanAnhSearchType>
              key="query"
              label="Tìm kiếm nhanh"
              name="query"
            >
              <Input placeholder="Tên, mã số DN, SĐT, email..." />
            </Form.Item>
          </Col>
          <Col xl={6} lg={8} md={12} xs={24}>
            <Form.Item<VuViecPhanAnhSearchType>
