...
interface SpecialistActionProps {
...
const SpecialistAction: React.FC<SpecialistActionProps> = ({
  item,
  roles,
  onTransitionClick,
  onlyButtons = false,
  ...
  const loadSpecialists = useCallback(async () => {
    ...
    try {
      ...
      if (item.platformManageTypeId !== "NTThongBaoKD" && hasPermission("PLATFORMBANENTANG_MANAGE_ACTION_PHANCONG")) {
  ...
  }, [item.platformManageTypeId, isAdmin, currentUser?.menuData]);
  ...
  const getActionButtons = () => {
    ...
    // Chờ duyệt (1) / Đề nghị chỉnh sửa (2)
      ...
      if (!isAssigned) {
        ...
        if (canProcess) {
          if (canActionDuyetDienTu) {
            buttons.push(
              <Button key="duyet" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaDuyetDienTu, title: "Duyệt điện tử hồ sơ", buttonColor: "primary" })}>{`${item.platformManageTypeId === "NTThongBaoKD" ? "Trình trưởng phòng" : "Duyệt điện tử hồ sơ"}`}</Button>
            );
          }
          if (canActionBoSungCV) {
            buttons.push(
              <Button key="bosung" type="primary" icon={<ExclamationCircleOutlined />} style={warningButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.CanBoSungThongTin, title: "Yêu cầu bổ sung thông tin", buttonColor: "warning" })}>{`${item.platformManageTypeId === "NTThongBaoKD" ? "Yêu cầu bổ sung thông tin" : "Yêu cầu bổ sung thông tin"}`}</Button>
            );
          }
        }
    ...
    if (item.status === PlatformStatusConstant.DangXinYKien && (!isAssigned || canProcess)) {
      if (canActionDuyetDienTu) {
        buttons.push(
          <Button key="duyet" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaDuyetDienTu, title: "Duyệt điện tử hồ sơ", buttonColor: "primary" })}>Duyệt điện tử</Button>
        );
      }
      if (canActionBoSungCV) {
        buttons.push(
          <Button key="bosung" type="primary" icon={<ExclamationCircleOutlined />} style={warningButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.CanBoSungThongTin, title: "Yêu cầu bổ sung thông tin", buttonColor: "warning" })}>Yêu cầu bổ sung</Button>
        );
      }
    }

    // Đã duyệt điện tử (4)
    if (item.status === PlatformStatusConstant.DaDuyetDienTu
      || (item.platformManageTypeId == "NTThongBaoKD"
        && ((canActionReviewThongQua && item.status == PlatformStatusConstant.DaDuyetDienTu)
          || (canActionDuyetDienTu && canActionReviewThongQua && (item.status == PlatformStatusConstant.ChoDuyet || item.status == PlatformStatusConstant.DeNghiChinhSua))))) {
      const isOwnSpecialist = isCurrentAssignedSpecialist && !isAdmin;
      if (!isOwnSpecialist) {
        if (canActionReviewThongQua && item.reviewId) {
          buttons.push(
            <Button key="review" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaReview, title: "Xác nhận Review (Trình Lãnh đạo Sở phê duyệt)", buttonColor: "primary" })}>Trình lãnh đạo sở</Button>
          );
          if (canActionPheDuyet) {

            buttons.push(
              <Button key="pheduyet" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaXacNhan, title: "PHÊ DUYỆT CHÍNH THỨC & CẤP MÃ NỀN TẢNG TMĐT", buttonColor: "primary" })}>Duyệt kết thúc</Button>
            );
          }
        }
        // if (canActionYeuCauBanGiay && item.platformManageTypeId !== "NTThongBaoKD") {
        //   buttons.push(
        //     <Button key="giay" type="primary" icon={<SolutionOutlined />} style={infoButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.CanBanGiay, title: "Yêu cầu nộp bản giấy đối chiếu", buttonColor: "warning" })}>Yêu cầu bản giấy</Button>
        //   );
        // }
    ...
    // Đã review (26)
    if (item.status === PlatformStatusConstant.DaReview || (item.platformManageTypeId == "NTThongBaoKD" && ((canActionPheDuyet && item.status == PlatformStatusConstant.DaReview) || (canActionDuyetDienTu && canActionPheDuyet && (item.status == PlatformStatusConstant.ChoDuyet || item.status == PlatformStatusConstant.DeNghiChinhSua))))) {
      const isOwnSpecialist = isCurrentAssignedSpecialist && !isAdmin;
      if (!isOwnSpecialist) {
        if (canActionPheDuyet && item.reviewId && ((item.status == PlatformStatusConstant.DaReview && !canActionReviewThongQua) || ((item.status == PlatformStatusConstant.ChoDuyet || item.status == PlatformStatusConstant.DeNghiChinhSua) && canActionDuyetDienTu))) {
          buttons.push(
            <Button key="pheduyet" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaXacNhan, title: "PHÊ DUYỆT CHÍNH THỨC & CẤP MÃ NỀN TẢNG TMĐT", buttonColor: "primary" })}>Duyệt kết thúc</Button>
          );
        }
        ...
        if (item.platformManageTypeId == "NTThongBaoKD" && canActionPheDuyet && (item.status == PlatformStatusConstant.ChoDuyet || item.status == PlatformStatusConstant.DeNghiChinhSua)) {
          buttons.push(
            <Button key="pheduyet" type="primary" icon={<CheckCircleOutlined />} style={successButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.DaXacNhan, title: "PHÊ DUYỆT CHÍNH THỨC & CẤP MÃ NỀN TẢNG TMĐT", buttonColor: "primary" })}>Duyệt kết thúc</Button>
          );
        }
    ...
    if (canRejectAny && isNotTerminal) {
      buttons.push(
        <Button key="tuchoi" type="primary" icon={<CloseCircleOutlined />} style={dangerButtonStyle} onClick={() => handleActionClick({ targetStatus: PlatformStatusConstant.BiTuChoi, title: item.status == PlatformStatusConstant.DeNghiChinhSua ? "Từ chối đề nghị chỉnh sửa" : "Từ chối hồ sơ đăng ký", buttonColor: "danger" })}>{item.status == PlatformStatusConstant.DeNghiChinhSua ? "Từ chối đề nghị chỉnh sửa" : "Từ chối"}</Button>
      );
    }
