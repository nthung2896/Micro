...
const CreateOrUpdate: React.FC<Props> = (props: Props) => {
  ...
  const loadParentOptions = async () => {
    try {
      const r = await navMenuService.getData({ pageIndex: 1, pageSize: 1000, menuType: "MAIN" });
      if (r.status && r.data) {
  ...
  const handleOnFinish: FormProps<NavMenuRequest>["onFinish"] = async (
    ...
    try {
      if (props.menuItem) {
        const response = await navMenuService.update({ ...formData, id: props.menuItem.id, menuType: "MAIN" });
        if (response.status) {
        ...
        const response = await navMenuService.create({ ...formData, menuType: "MAIN" });
        if (response.status) {
  ...
  return (
    <Modal
      title={
        props.menuItem != null ? "Chỉnh sửa menu" : "Thêm mới menu"
      }
      open={isOpen}
      onOk={() => form.submit()}
    ...
    >
      ...
      >
        ...
        >
          <Input placeholder="Ví dụ: Trang chủ, Tin tức..." maxLength={250} />
        </Form.Item>
        ...
        >
          <Input placeholder="Ví dụ: /, /tin-tuc, /van-ban-phap-luat..." maxLength={500} />
        </Form.Item>
