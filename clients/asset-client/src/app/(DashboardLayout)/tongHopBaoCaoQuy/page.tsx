...
export default function TongHopBaoCaoQuyPage() {
  ...
  const fetchData = async (
    ...
    const currentQuarterValue = month ?? filterQuarter;
    ...
    if (!currentQuarterValue && !currentName && !currentTaxCode) {
      message.warning(
    ...
    try {
      const body: any = {
        ...
        thangBaoCao: currentQuarterValue,
      ...
      const isFullSearch =
        currentYear && (currentQuarterValue || currentName || currentTaxCode);
      if (!isFullSearch) {
  ...
  const idDoiTuongBaoCao =
   ...
   const thang = filterQuarter ? data[0]?.thangBaoCao : null;
    ...
    try {
      ...
      a.href = url;
      a.download = "Kết quả hoạt động vận chuyển, giao nhận (Quý).xlsx";
      a.click();
