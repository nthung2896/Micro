...
const ManagerViewReportDetailPage: React.FC<PageProps> = ({ params, searchParams }) => {
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Button
          ...
          icon={<EyeOutlined />}
          onClick={() => {
            const url = `/cauHinhBieuMauBaoCao/${id}/xemBaoCao/${baoCaoDoiTuongId}/${record.id}${month ? `?month=${month}&year=${year}` : ""}`;
            router.push(url);
          }}
        >
          Xem biểu mẫu
  ...
  return (
    <>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <Button
            ...
            onClick={() => router.push(`/cauHinhBieuMauBaoCao/${id}`)}
      ...
      <Card title={`Danh sách Biểu mẫu doanh nghiệp đã nộp ${month ? `(Tháng ${month}/${year})` : ""}`} bordered={false}>
        <Table
