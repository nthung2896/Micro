...
const KPI_NhomTieuChiImportModal: React.FC<ImportModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  initialFile,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialFile) {
        ...
        // Also call checkFileWorksheet if there is an initial file
  ...
  const handleImport = async (values: any) => {
    ...
    const payload: KPI_BoTieuChiDonViImportVMType = {
      ...
      TotalColumns: values.TotalColumns ?? 7,
      WorkSheetName: values.WorkSheetName,
      StartCol: headerData?.startCol ?? 1,
      Headers: headerData?.headers ?? [],
    } as any; // Type assertion since WorkSheetName, StartCol, Headers might not be in the original type
    ...
    setImportResult(null);
    try {
      const isVuDonVi = values.importMode === "vu_don_vi";
      const res = isVuDonVi
        ? await kPI_NhomTieuChiService.importExcelVuDonVi(file, payload)
        : await kPI_NhomTieuChiService.importExcelDirect(file, payload);
  ...
  return (
    <Modal
      title="Import dữ liệu Nhóm Tiêu Chí KPI (1-N Bộ Tiêu Chí Đơn Vị)"
      open={isOpen}
      onCancel={onClose}
      footer={[
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
            <Form.Item
              name="importMode"
              label={<span className="font-semibold text-slate-800">Chọn cấu trúc file Excel:</span>}
              className="mb-0"
            >
              <Radio.Group className="flex flex-col gap-2">
                <Radio value="vu_don_vi">
                  <span className="font-semibold text-blue-700">Mẫu mới (9 cột)</span>
                  <div className="text-xs text-slate-500 ml-6">
                    Mục chữ cái (A, B, C...) → Số La Mã (I, II...) → Nhóm nhiệm vụ (1, 2...) → Công việc (1.1, 1.2...) → Sản phẩm & Điểm số
                  </div>
                </Radio>
                <Radio value="standard">
                  <span className="font-medium text-slate-700">Mẫu cũ (7 cột)</span>
                  <div className="text-xs text-slate-500 ml-6">
                    Số La Mã (I, II...) → Số thường (1, 2...) → Công việc chi tiết → Sản phẩm đầu ra
                  </div>
                </Radio>
              </Radio.Group>
            </Form.Item>
...
export default KPI_NhomTieuChiImportModal;
