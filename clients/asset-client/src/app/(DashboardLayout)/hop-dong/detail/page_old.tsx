        ...
        >
          <div className="flex flex-row justify-between items-center mb-3 flex-wrap gap-3" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 12 }}>
            ...
            <div className="flex flex-row gap-2" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              {/* Workflow buttons */}
              {workflowActions.map((rule) => {
                const label = getDropdownLabel(actionOptions, rule.action, rule.fallbackLabel);
                const icon = rule.action === ContractActionConstant.PhanCongXuLy ? (
                  <UserAddOutlined />
                ) : rule.danger ? (
                  <StopOutlined />
                ) : (
                ...
                return (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      handleChangeStatus(rule.action, label);
                    }}
                  >
                    {label}
                  </Button>
