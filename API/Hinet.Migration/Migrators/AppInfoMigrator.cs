...
// Migrate dbo.AppInfo (SQL Server) → public."PlatformManage" (Postgres)
public class AppInfoMigrator : IMigrator
{
    ...
    private readonly ILogger<AppInfoMigrator> _log;

    public string Name => "AppInfo";

    public AppInfoMigrator(IConfiguration cfg, ILogger<AppInfoMigrator> log)
    {
    ...
    {
        _log.LogInformation("=== Bắt đầu migrate AppInfo (DryRun={DryRun}, Batch={Batch}, Limit={Limit}) ===",
        ...
        var totalOld = await oldDb.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM dbo.AppInfo");
        _log.LogInformation("Tổng dòng AppInfo cũ: {Total}", totalOld);
        ...
        while (true)
        {
            ...
            var batch = (await oldDb.QueryAsync<OldAppInfo>(@"
                SELECT TOP (@Take)
                    Id, Status, WebsiteTypeId, AppManageTypeId, TinhIdNew, OrganizationId,
                    TypeOrganization, DepartmentId, CompanyTypeId, CompanyName, CompanyTaxCode,
                    CompanyAddress, CompanyPhone, CompanyFax, Name, Domain, Customer,
                    MonitorMethod, ProductType, ISPId, ISPIdKhac, StaffNumber, CommitApprove,
                    Detail, History, SubmitDate, SubmitName, SubmitSignature, ReviewDate,
                    ...
                    Reason, Comment, Utility, DomainAdd, ListFunctionsUpgradeItemId, ImagePath,
                    StickId, TinhId, HuyenId, XaId, QuocGiaId, SendStatus, IsDelete,
                    DeleteById, DeleteDate, EndDateNoti, TienIchKhac, LoaiHangHoaKhac, DateLine,
                    RepresenterName, RepresenterMobile, RepresenterJob, RepresenterEmail,
                    Address, PhuongThucLienHe, RepresenterNameOnline, RepresenterJobOnline,
                    RepresenterEmailOnline, AddressOnline,
                    PhuongThucLienHeOnline, RepresenterNameDaiDien, RepresenterJobDaiDien,
                    RepresenterMobileDaiDien, RepresenterEmailDaiDien, AddressDaiDien,
                    PhuongThucLienHeDaiDien, IsUuTienGuiBaoCao, ReviewDatHang, UserReviewDatHang,
                    API, URLApp, UserApp, PassApp, IsNuocNgoai, SoCongVan,
                    PhuongAnCungCapBaoCao, CCCD, RepresenterCCCD, DataSignedTruongPhong,
                    SignDateTruongPhong, TruongPhongId, DataSignedLanhDao, SignDateLanhDao,
                    LanhDaoId, DataSignedUser, SignDateUser,
            ...
            foreach (var old in batch)
            {
                ...
                try
                {
                    ...
                    await newDb.ExecuteAsync(@"
                        ...
                        new
                        {
                            Id = newId,
                            old.Status,
                            AppManageTypeId = old.AppManageTypeId switch
                            {
                                0 => 1,
                                1 => 3,
                                _ => old.AppManageTypeId
                            },
                            ...
                            URLApp = Clean(old.URLApp),
                            UserApp = Clean(old.UserApp),
                            PassApp = Clean(old.PassApp),
                            ...
                            RepresenterCCCD = Clean(old.RepresenterCCCD ?? old.CCCD), // Ưu tiên RepresenterCCCD rồi đến CCCD
                    ...
                    await newDb.ExecuteAsync(@"
                        INSERT INTO _migration.app_id_map (old_id, new_id)
                        VALUES (@OldId, @NewId)",
                ...
                {
                    errors++;
                    _log.LogError(ex, "Lỗi migrate AppInfo Id={OldId}", old.Id);
                    if (!_dryRun)
                    {
            ...
            progress?.Report(new MigrationProgress("AppInfo", totalOld, processed, inserted, skipped, errors,
                $"lastId={lastId}"));
        }

        progress?.Report(new MigrationProgress("AppInfo", totalOld, totalOld, inserted, skipped, errors, "✅ Done"));
        _log.LogInformation(
            "=== Hoàn tất AppInfo: processed={Processed} inserted={Inserted} skipped={Skipped} errors={Errors} ===",
    ...
    private static async Task LogErrorAsync(IDbConnection newDb, long oldId, string reason)
    {
        try
        {
            await newDb.ExecuteAsync(@"
                ...
                VALUES (@T, @Id, @Reason)",
                new { T = "AppInfo", Id = oldId, Reason = reason });
        }
        catch { }
    }
}
