...
public class ContractFileMigrator : IMigrator
{
    private readonly string _oldConnStr;
    private readonly string _newConnStr;
    private readonly int _batchSize;
    private readonly bool _dryRun;
    private readonly ILogger<ContractFileMigrator> _log;

    public string Name => "ContractFile";

    public ContractFileMigrator(IConfiguration cfg, ILogger<ContractFileMigrator> log)
    {
    ...
    {
        _log.LogInformation("=== Bắt đầu migrate ContractFile (DryRun={DryRun}, Batch={Batch}) ===",
            _dryRun, _batchSize);
        ...
        var totalOld = await oldDb.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM dbo.ContractInfoFileAttach");
        ...
        while (true)
        {
            ...
            var batch = (await oldDb.QueryAsync<OldContractFile>(@"
                SELECT TOP (@Take)
                    Id, ContractId, Name, FileName, Note, Type,
                    CreatedDate, CreatedBy, CreatedID,
                    UpdatedDate, UpdatedBy, UpdatedID
                FROM dbo.ContractInfoFileAttach
                WHERE Id > @LastId
                ORDER BY Id",
            ...
            foreach (var old in batch)
            {
                ...
                try
                {
                    ...
                    var existingNewId = await newDb.ExecuteScalarAsync<Guid?>(
                        @"SELECT new_id FROM _migration.contract_file_id_map WHERE old_id=@id",
                        new { id = old.Id });
                    ...
                    var contractId = await newDb.ExecuteScalarAsync<Guid?>(
                        @"SELECT new_id FROM _migration.authcontract_id_map WHERE old_id=@id",
                        new { id = old.ContractId });

                    if (!contractId.HasValue)
                    {
                        missingContract++;
                        await LogErrorAsync(newDb, old.Id, $"Không tìm thấy AuthenticationContract mapping cho ContractId={old.ContractId}");
                        continue;
                    }
                    ...
                    await newDb.ExecuteAsync(@"
                        ...
                        new
                        {
                            ...
                            LoaiTaiLieu = Truncate(GetLoaiTaiLieu(old.Name, old.Note), 250),
                            ItemId = contractId.Value,
                    ...
                    await newDb.ExecuteAsync(@"
                        INSERT INTO _migration.contract_file_id_map (old_id, new_id)
                        VALUES (@OldId, @NewId)",
                ...
                {
                    errors++;
                    _log.LogError(ex, "Lỗi migrate ContractFile Id={OldId}", old.Id);
                    if (!_dryRun)
                    {
            ...
            progress?.Report(new MigrationProgress("ContractFile", totalOld, processed, inserted, skipped, errors,
                $"lastId={lastId}"));
            _log.LogInformation(
        ...
        progress?.Report(new MigrationProgress("ContractFile", totalOld, totalOld, inserted, skipped, errors, "✅ Done"));
        _log.LogInformation(
            "=== Hoàn tất ContractFile: processed={Processed} inserted={Inserted} skipped={Skipped} errors={Errors} missingContract={MissingContract} ===",
            processed, inserted, skipped, errors, missingContract);
    }

    private static string GetLoaiTaiLieu(string? name, string? note)
    {
        var searchStr = $"{name} {note}".ToLower();
        if (searchStr.Contains("đơn đăng ký") || searchStr.Contains("dondangky"))
            return "DangKyChungThuc";
        if (searchStr.Contains("đề án") || searchStr.Contains("dean") || searchStr.Contains("hướng dẫn") || searchStr.Contains("huongdan"))
            return "HuongDanDichVu";
        if (searchStr.Contains("tên miền") || searchStr.Contains("tenmien"))
            return "ChungMinhTenMien";

        return !string.IsNullOrWhiteSpace(note) ? note.Trim() : "ContractDocument";
    }
    ...
    private static async Task LogErrorAsync(IDbConnection newDb, long oldId, string reason)
    {
        try
        {
            await newDb.ExecuteAsync(@"
                ...
                VALUES (@T, @Id, @Reason)",
                new { T = "ContractFile", Id = oldId, Reason = reason });
        }
